...
@Injectable()
export class EquipmentMaintenanceModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      let isTextInMISSION_DESCRIPTION = (item: any) =>
        item.MISSION_DESCRIPTION ? item.MISSION_DESCRIPTION.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;


      let isTextInMISSION_TYPE = (item: any) =>
        item.MISSION_TYPE ? item.MISSION_TYPE.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

      this.displayData = this.savedData.filter(
        item => isTextInMISSION_DESCRIPTION(item) || isTextInMISSION_TYPE(item) ,
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Equipment_Maintenance_Status_Report.xlsx');
    })
  }
