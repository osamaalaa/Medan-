...
@Injectable()
export class PurchasingOrderModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      let isTextInMAX_QUANTITY = (item: any) =>
        item.MAX_QUANTITY ? item.MAX_QUANTITY.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

      let isTextInMIN_QUANTITY = (item: any) =>
        item.MIN_QUANTITY ? item.MIN_QUANTITY.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

      this.displayData = this.savedData.filter(
        item => isTextInEN_NAME(item) || isTextInEN_DESCRIPTION(item) || isTextInMAX_QUANTITY(item) || isTextInMIN_QUANTITY(item),
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Items_reorder_limit_Report.xlsx');
    })
  }
