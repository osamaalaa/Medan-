...
@Injectable()
export class DailyWasteModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      this.displayData = this.savedData.filter(
        item => isTextInACTIVITY_TITLE_EN(item) || isTextInACTIVITY_TYPE_EN(item) || isTextInACTION_TYPE_EN(item) || isTextInTEMPLATE_NAME_EN(item),
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Volume_of_Waste_Report.xlsx');
    })
  }
