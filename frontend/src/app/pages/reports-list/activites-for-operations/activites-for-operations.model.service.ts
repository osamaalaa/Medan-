...
@Injectable()
export class ActivitesForOperationsModelService extends TableBase {
    constructor() {
        super();
    }
    ...
    public searchItems(searchText: string): void {
        ...
        if (searchText) {
            ...
            this.displayData = this.savedData.filter(
                item => isTextInASSET_NAME(item) || isTextInRECURING_EN(item) || isTextInJOB_ORDER_DESC(item),
            );
        } else {
 ...
 generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
   ...
   // worksheet.getColumn(3).width = 20;
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Work_Orders_By_Maintenance_Type_Report.xlsx');
    })
  }
