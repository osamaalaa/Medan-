...
@Injectable()
export class EmployeesIssuesModelService extends TableBase {
    constructor() {
        super();
    }
    ...
    public searchItems(searchText: string): void {
        ...
        if (searchText) {
            ...
            let isTextInVIOLATION_NAME_EN = (item: any) =>
                item.VIOLATION_NAME_EN ? item.VIOLATION_NAME_EN.toString()
                    .toLowerCase()
                    .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

                    let isTextInVIOLATION_VALUE = (item: any) =>
                    item.VIOLATION_VALUE ? item.VIOLATION_VALUE.toString()
                        .toLowerCase()
                        .indexOf(searchText.toString().toLowerCase()) !== -1 : false;
            ...
            this.displayData = this.savedData.filter(
                item => isTextInEMP_NAME_EN(item) || isTextInVIOLATION_NAME_EN(item) || isTextInVIOLATION_VALUE(item) || isTextInISSUE_SUMMARY(item),
            );
        } else {
...
generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Employees_Violations_Report.xlsx');
    })
  }
