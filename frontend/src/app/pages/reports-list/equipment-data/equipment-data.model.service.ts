...
@Injectable()
export class EquipmentDataModelService extends TableBase {
    constructor() {
        super();
    }
    ...
    public searchItems(searchText: string): void {
        ...
        if (searchText) {
            ...
            let isTextInASSET_SERIAL = (item: any) =>
                item.ASSET_SERIAL ? item.ASSET_SERIAL.toString()
                    .toLowerCase()
                    .indexOf(searchText.toString().toLowerCase()) !== -1 : false;
            ...
            let isTextInASSET_TYPE_EN = (item: any) =>
                item.ASSET_TYPE_EN ? item.ASSET_TYPE_EN.toString()
                    .toLowerCase()
                    .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

            this.displayData = this.savedData.filter(
                item => isTextInASSET_NAME(item) || isTextInASSET_SERIAL(item) || isTextInDESCRIPTION(item) || isTextInASSET_TYPE_EN(item),
            );
        } else {
...
generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Equipment-Data Report.xlsx');
    })
  }
