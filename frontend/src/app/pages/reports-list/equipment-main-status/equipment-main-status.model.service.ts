...
@Injectable()
export class EquipmentMainStatusModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      this.displayData = this.savedData.filter(
        item => isTextInASSET_NAME(item) || isTextInSTATUS(item) ,
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Equipment_Operation_Status_Report.xlsx');
    })
  }
