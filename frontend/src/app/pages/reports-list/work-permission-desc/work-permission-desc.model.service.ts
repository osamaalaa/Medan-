...
@Injectable()
export class WorkPermissionDescModelService extends TableBase {
    constructor() {
        super();
    }
    ...
    public searchItems(searchText: string): void {
        ...
        if (searchText) {
            let isTextInPERMISSION_TYPE_EN = (item: any) =>
                item.PERMISSION_TYPE_EN ? item.PERMISSION_TYPE_EN.toString()
                    .toLowerCase()
                    .indexOf(searchText.toString().toLowerCase()) !== -1 : false;

            this.displayData = this.savedData.filter(
                item => isTextInPERMISSION_TYPE_EN(item)  ,
            );
        } else {
    ...
    generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
        ...
        workbook.xlsx.writeBuffer().then((data) => {
            ...
            fs.saveAs(blob, 'Work-PermissionsDescReport.xlsx');
        })
    }
