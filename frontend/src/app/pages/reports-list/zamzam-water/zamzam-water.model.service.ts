...
@Injectable()
export class ZamZamWaterModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      let isTextInACTIVITY_VALUE = (item: any) =>
        item.ACTIVITY_VALUE ? item.ACTIVITY_VALUE.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;
      ...
      this.displayData = this.savedData.filter(
        item => isTextInACTIVITY_TITLE_EN(item) || isTextInACTIVITY_VALUE(item) || isTextInTEMPLATE_NAME_EN(item) || isTextInMINUTE_OR_EQUIB_CATEGORY(item),
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Zamzam_Water_Withdraw_Report.xlsx');
    })
  }
