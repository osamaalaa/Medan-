...
@Injectable()
export class IncidentsModelService extends TableBase {
  constructor() {
    super();
  }
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      let isTextInEMPLOYEE_ASSIGNED_EN = (item: any) =>
        item.EMPLOYEE_ASSIGNED_EN ? item.EMPLOYEE_ASSIGNED_EN.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1 : false;
      ...
      this.displayData = this.savedData.filter(
        item => isTextInISSUE_TITLE(item) ||
          isTextInEMPLOYEE_ASSIGNED_EN(item) ||
          isTextInISSUE_SUMMARY(item)
      );
    } else {
  ...
  generateExcel(mainData, mainHead, parameters, dateValue, titleAr, date) {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      fs.saveAs(blob, 'Incident_Data_in_Location_or_Shift_Report.xlsx');
    })
  }
