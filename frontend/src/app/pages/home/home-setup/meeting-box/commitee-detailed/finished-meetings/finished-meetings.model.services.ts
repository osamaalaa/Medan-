...
@Injectable()
export class FinishedMeetingsModelService extends TableBase {
  ...
  public searchItems(searchText: string): void {
    ...
    if (searchText) {
      ...
      let isTextInMEETING_DATE = (item: any) =>
        item.MEETING_DATE.toString()
          .toLowerCase()
          .indexOf(searchText.toString().toLowerCase()) !== -1;


      this.displayData = this.savedData.filter(
        item => isTextInMEETING_DESC(item) || isTextInMEETING_DATE(item) ,
      );
    } else {
