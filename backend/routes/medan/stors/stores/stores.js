...
router.get('/selectOnestores/:STORES_ID', function (req, res) {
  ...
  businessPool(req, res, statements.selectOnestores.statment, { 'STORES_ID': req.params.STORES_ID }).then(data => {
...
router.post('/SearchingForStoresByName', function (req, res) {
  servicePool(req, res, statements.SearchingForStoresByName.statment,
...
router.get('/selectAllstores', (req, res) => {
  ...
  en_name,
  parent_stores_id,
  (SELECT AR_NAME  FROM INVENTORY.stores PS WHERE PS.stores_id = I.parent_stores_id) PARENT_STORE_AR_NAME,
  (SELECT EN_NAME  FROM INVENTORY.stores PS WHERE PS.stores_id = I.parent_stores_id) PARENT_STORE_EN_NAME,
  store_type, 
  issue_policy,
  profit_margin,
  subsidiary_id,
